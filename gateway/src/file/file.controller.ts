...
export class FileController {
  ...
  > {
    ...
    for (const file of filesImport) {
      const message = `${file.name} imported successfully`;
      const dtoNotification: CreateNotificationRequest = {
        recipientId: req.user.sub,
        recipientType: TypeRecipient.USER,
        type: TypeNotification.IMPORT_FILE,
  ...
  > {
    const files = await this.fileService.getFilesBin(req.user.sub, {
      page: page,
      sort: sort,
      limit: limit,
      order: order,
