...
public class NotificationService {
    ...
    public NotificationDto createNotification(CreateNotificationRequest request) {
        ...
        notification.getMetadata().putIfAbsent("title", template.getMessage());
        Notification notif = this.repository.save(notification);
        System.out.println(notif);
        notif.getMetadata().putIfAbsent("id", notif.getId());
        notif = this.repository.save(notif);
        System.out.println(notif);
        if (channel == TypeChannel.EMAIL) {
            handler.sendNotification(notification.getType(), notif.getTypeChannel(), notif.getId(), notif.getMetadata());
        }
        if (channel == TypeChannel.PUSH) {
            String token = (String) notif.getMetadata().get("device-token");
